...
class DBTableCompare(models.Model):
    ...
    added_on = models.DateTimeField(auto_now_add=True, blank=True)
    last_compared_file_loc = models.CharField(max_length=1000, blank=True, default=None, null=True)
...
class DBCompareDBResults(models.Model):
    id = models.AutoField(primary_key=True)
...
class DBTableColumnCompare(models.Model):
    ...
    added_on = models.DateTimeField(auto_now_add=True, blank=True)
    src_dst = models.CharField(max_length=10, choices=DBSrcDst.choices, blank=True, null=True)
    ...
    class Meta:
        indexes = [
            models.Index(fields=['table_name', 'compare_dbs', 'column_name', 'added_on', 'src_dst']),
        ]
...
class DBTableCompareTokens(models.Model):
    id = models.AutoField(primary_key=True)
    token = models.CharField(max_length=100, blank=False)
    file_path = models.CharField(max_length=1000, blank=False)
