...
P_COUNT_ROWS_Q = '''SELECT count(*) from %(TABLE)s'''
...
O_ROW_COUNT = '''SELECT UPPER(table_name), num_rows FROM all_tables WHERE owner = :SCH'''
...
P_ROW_COUNT2 = '''SELECT
    UPPER(pgClass.relname)   AS tableName,
...
WHERE
        ...
        pgClass.relkind='r'
    and pgNamespace.nspname = %(SCHEMA)s'''
